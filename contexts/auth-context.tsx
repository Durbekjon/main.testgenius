...
import { TOKEN_KEYS, isAuthenticated, logout as authLogout, UserData } from '@/lib/auth';
...
interface AuthContextType {
  ...
  verifyEmail: (token: string, code: string) => Promise<{ success: boolean; error?: string }>;
  logout: () => void;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const register = async (userData: { firstName: string; lastName?: string; email: string; password: string }): Promise<{ success: boolean; error?: string }> => {
    try {
  ...
  const verifyEmail = async (token: string, code: string): Promise<{ success: boolean; error?: string }> => {
    try {
